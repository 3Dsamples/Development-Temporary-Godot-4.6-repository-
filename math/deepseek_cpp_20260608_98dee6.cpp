...
//File 0077 : core/math/morton.h
//Morton code (Z‑order curve) encoding/decoding for 2D/3D, 32/64‑bit, with SIMD batch operations and neighbour arithmetic.

#ifndef ORTHOTREE_CORE_MATH_MORTON_H_INCLUDED
#define ORTHOTREE_CORE_MATH_MORTON_H_INCLUDED
...
// ============================================================================
//  32‑bit Morton (2D, 10 bits per component)
// ============================================================================
namespace Morton32 {
...
// ============================================================================
//  64‑bit Morton (2D: 21 bits/comp, 3D: 21 bits/comp)
// ============================================================================
namespace Morton64 {
    // 2D
...
// ============================================================================
//  Generic dimension‑aware encoding (using floating point quantisation)
// ============================================================================
template<Dimension Dim, typename T = float>
uint64_t mortonEncode(const Basic::Vector<T, Dim>& point,
                      const Basic::Vector<T, Dim>& minBound,
                      const Basic::Vector<T, Dim>& maxBound,
                      uint32_t bitsPerCoord = 21) {
    Basic::Vector<T, Dim> t = (point - minBound) / (maxBound - minBound);
...
template<Dimension Dim, typename T = float>
Basic::Vector<T, Dim> mortonDecode(uint64_t code,
                                   const Basic::Vector<T, Dim>& minBound,
                                   const Basic::Vector<T, Dim>& maxBound,
                                   uint32_t bitsPerCoord = 21) {
    if constexpr (Dim == Dim2) {
        ...
        T maxVal = static_cast<T>((uint64_t(1) << bitsPerCoord) - 1);
        T tX = static_cast<T>(x) / maxVal;
        T tY = static_cast<T>(y) / maxVal;
        return Basic::Vector<T,2>(minBound[0] + tX * (maxBound[0] - minBound[0]),
        ...
        T tX = static_cast<T>(x) / maxVal;
        T tY = static_cast<T>(y) / maxVal;
        T tZ = static_cast<T>(z) / maxVal;
        return Basic::Vector<T,3>(minBound[0] + tX * (maxBound[0] - minBound[0]),
                                  minBound[1] + tY * (maxBound[1] - minBound[1]),
...
// ============================================================================
//  Neighbour cell stepping
// ============================================================================
inline uint64_t mortonNextX(uint64_t code) noexcept { return code + 0x1249249249249249ULL; }
inline uint64_t mortonNextY(uint64_t code) noexcept { return code + 0x2492492492492492ULL; }
inline uint64_t mortonNextZ(uint64_t code) noexcept { return code + 0x4924924924924924ULL; }
inline uint64_t mortonPrevX(uint64_t code) noexcept { return code - 0x1249249249249249ULL; }
inline uint64_t mortonPrevY(uint64_t code) noexcept { return code - 0x2492492492492492ULL; }
inline uint64_t mortonPrevZ(uint64_t code) noexcept { return code - 0x4924924924924924ULL; }

// Parent / child (octree)
inline uint64_t mortonParent(uint64_t code) noexcept { return code >> 3; }
inline uint64_t mortonChild(uint64_t code, uint8_t childIdx) noexcept { return (code << 3) | (childIdx & 0x7); }
inline uint8_t mortonChildIndex(uint64_t code, uint32_t depth) noexcept {
    return (code >> (3 * depth)) & 0x7;
}

// Common prefix length (bits)
inline uint32_t mortonCommonPrefixBits(uint64_t a, uint64_t b) noexcept {
    uint64_t diff = a ^ b;
    if (diff == 0) return 64;
...
#endif // ORTHOTREE_CORE_MATH_MORTON_H_INCLUDED