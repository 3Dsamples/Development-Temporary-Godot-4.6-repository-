// File 227: modules/newton/src/solver/newton_solver.cpp
// NewtonSolver – sequential impulse solver with warm‑starting, Baumgarte
// stabilisation, friction, restitution, and joint solving. Operates on
// islands built by NewtonIsland.
...
void NewtonSolver::solve_island(NewtonIsland *island, real_t dt) {
    ...
    int n = iterations;
    // Warm‑starting: first iteration applies stored impulses, then updates.
    bool apply_position = false;
    for (int iter = 0; iter < n; ++iter) {
        // Velocity-level corrections
        for (NewtonContactPoint &cp : const_cast<LocalVector<NewtonContactPoint>&>(contacts)) {
            ...
            // Restitution only in first iteration
            real_t restitution = (iter == 0) ? cp.restitution : 0.0f;
            real_t target_dv = -(1.0f + restitution) * vn;

            // Position correction (Baumgarte) in later iterations
        ...
        // Joint constraints (velocity + position)
