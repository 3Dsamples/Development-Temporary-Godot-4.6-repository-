...
// Vienna Slider (Prismatic) Joint – constrains two bodies to translate only
// along a single axis, with optional limits and motor.  The joint enforces
// the pivot points to stay aligned perpendicular to the slider axis and
// removes all relative rotation.

#ifndef VIENNA_JOINTS_SLIDER_H
#define VIENNA_JOINTS_SLIDER_H
...
public:
	...
	// --- Translation limits (relative to initial offset) ---
...
protected:
	static void _bind_methods() {
		...
		ClassDB::bind_method(D_METHOD("set_pivot", "pivot"), &ViennaSliderJoint::set_pivot);
		ClassDB::bind_method(D_METHOD("get_pivot"), &ViennaSliderJoint::get_pivot);
...
private:
	...
	real_t initial_offset = 0.0; // computed on first solve
...
inline void ViennaSliderJoint::solve(ViennaBody *a, ViennaBody *b, real_t dt) {
	...
	// ---- Restore initial offset (first solve) ----
	if (first_solve) {
	...
	if (limit_enabled) {
		// Current displacement relative to initial offset
