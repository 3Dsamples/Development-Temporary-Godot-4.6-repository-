...
// Vienna Hinge Joint – constrains two bodies to rotate around a common axis,
// with optional limits and motor.  The joint enforces pivot coincidence and
// axis alignment using position‑level Baumgarte stabilisation and velocity‑level
// impulses.

#ifndef VIENNA_JOINTS_HINGE_H
#define VIENNA_JOINTS_HINGE_H
...
// ---------------------------------------------------------------------------
// Inline solve implementation (full logic)
// ---------------------------------------------------------------------------
inline void ViennaHingeJoint::solve(ViennaBody *a, ViennaBody *b, real_t dt) {
	...
	vec3 worldAnchorB = xB.xform(pivot_a); // using symmetric local pivot
	...
	// ---- Limit enforcement ----
	if (limit_enabled) {
		// Compute reference direction in body A perpendicular to the hinge axis
		vec3 refDirA = (Math::abs(worldAxisA.x) < 0.999f) ? 
			worldAxisA.cross(vec3(1, 0, 0)).normalized() : 
			worldAxisA.cross(vec3(0, 1, 0)).normalized();
		// The same reference direction expressed in body B's frame
		vec3 refDirB = xB.basis.xform(axis_a).normalized(); // not the reference; we need to compute the relative rotation around the axis
		// Better: we compute the signed angle between A and B around the hinge axis.
		// Use a helper direction: project body B's local X (or Z) onto the plane perpendicular to the axis.
		vec3 refB = xB.basis.get_column(0).normalized(); // local X of B
		refB = (refB - worldAxisB * refB.dot(worldAxisB)).normalized();
		vec3 refA = xA.basis.get_column(0).normalized();
		refA = (refA - worldAxisA * refA.dot(worldAxisA)).normalized();

		vec3 crossRefs = refB.cross(refA);
		real_t dotRefs = refB.dot(refA);
