// File 259: modules/newton/src/bodies/newton_body.cpp (final)
// NewtonBody implementation – includes damping setters/getters, inertia updates,
// force and impulse accumulation, integration, and all Godot bindings.
...
void NewtonBody::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_linear_damping", "damping"), &NewtonBody::set_linear_damping);
	ClassDB::bind_method(D_METHOD("get_linear_damping"), &NewtonBody::get_linear_damping);
	ClassDB::bind_method(D_METHOD("set_angular_damping", "damping"), &NewtonBody::set_angular_damping);
	ClassDB::bind_method(D_METHOD("get_angular_damping"), &NewtonBody::get_angular_damping);
...
void NewtonBody::set_transform(const mat4 &p_xform) { transform = p_xform; update_inverse_inertia(); }
...
void NewtonBody::set_mass(real_t p_mass) { mass = MAX(p_mass, 0.0); inverse_mass = (mass > 0.0) ? 1.0 / mass : 0.0; }
void NewtonBody::set_inertia(const mat3 &p_inertia) { inertia_local = p_inertia; inverse_inertia_local = inertia_local.inverse(); update_inverse_inertia(); }
void NewtonBody::update_inverse_inertia() { inverse_inertia_world = transform.basis * inverse_inertia_local * transform.basis.transposed(); }

void NewtonBody::set_linear_damping(real_t p_damp) { linear_damping = CLAMP(p_damp, 0.0, 1.0); }
real_t NewtonBody::get_linear_damping() const { return linear_damping; }
void NewtonBody::set_angular_damping(real_t p_damp) { angular_damping = CLAMP(p_damp, 0.0, 1.0); }
real_t NewtonBody::get_angular_damping() const { return angular_damping; }
...
void NewtonBody::clear_forces() { force_accum = vec3(); torque_accum = vec3(); }
