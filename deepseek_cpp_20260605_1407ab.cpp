//File 0008 (UPDATED) : core/xreducer.hpp
//High-performance reducer engine with SIMD accumulation, parallel execution policies, memory pool awareness, and full reduction semantics.
...
{
    /********************************************
     * Memory pool for temporary reduction buffers
     ********************************************/
    namespace detail
    {
        template <class T>
        class reduction_memory_pool
        {
        public:
            static reduction_memory_pool& instance()
            {
                static reduction_memory_pool pool;
                return pool;
            }
            ...
            T* allocate(std::size_t count)
            {
                ...
                if (!m_pool.empty() && m_pool.back().size() >= count)
                {
                ...
                T* ptr = buf.get();
                m_allocated.push_back(std::move(buf));
                return ptr;
            }
    ...
    namespace detail
    {
        struct plus
        {
            ...
            constexpr auto operator()(const T1& a, const T2& b) const { return a + b; }
            template <class T>
            constexpr auto simd_apply(const T& a, const T& b) const { return a + b; }
        };
        ...
        {
            template <class T1, class T2>
            constexpr auto operator()(const T1& a, const T2& b) const { return a > b ? a : b; }
            template <class T>
            constexpr auto simd_apply(const T& a, const T& b) const { return xt_simd::select(a > b, a, b); }
        };
    ...
    public:
        ...
        xreducer_functor(const F& f, const InitF& init_f) : m_f(f), m_init_f(init_f) {}
    ...
    public:
        ...
        template <class It> const_reference element(It first, It last) const;
    ...
    private:
        ...
        value_type reduce_1d(const value_type* data, size_type count) const;
    };

    template <class F, class E, class X, class ES>
    inline xreducer<F, E, X, ES>::xreducer(F&& f, E&& e, X&& axes, ES&& es) noexcept
        : m_f(std::forward<F>(f)), m_e(std::forward<E>(e)), m_axes(std::forward<X>(axes)), m_es(std::forward<ES>(es))
    {
    ...
    template <class F, class E, class X, class ES>
    inline auto xreducer<F, E, X, ES>::size() const -> size_type { return compute_size(m_shape); }

    template <class F, class E, class X, class ES>
    inline auto xreducer<F, E, X, ES>::shape() const -> shape_type { return m_shape; }
    ...
    inline auto xreducer<F, E, X, ES>::compute_shape() const -> shape_type
    {
        ...
        if (m_axes.empty()) return shape_type{};
        shape_type res;
    ...
    inline auto xreducer<F, E, X, ES>::reduce_over_axes_simd() const -> value_type
    {
        ...
        return reduce_1d(data, m_e.size());
    }
    ...
    inline auto xreducer<F, E, X, ES>::reduce_over_axes_parallel() const -> value_type
    {
        ...
        if constexpr (std::is_same_v<ES, parallel_strategy>)
        {
            ...
            for (auto& p : partials) result = m_f(result, p);
            return result;
        }
        else return reduce_over_axes_sequential();
    }

    template <class F, class E, class X, class ES>
    inline auto xreducer<F, E, X, ES>::reduce_over_axes_sequential() const -> value_type
    {
        return reduce_1d(m_e.data(), m_e.size());
    }
    ...
    inline auto xreducer<F, E, X, ES>::reduce_1d(const value_type* data, size_type count) const -> value_type
    {
        ...
        if constexpr (is_simd_enabled_v<value_type>)
        {
            ...
            for (std::size_t i = 0; i < simd_size; ++i) result = m_f(result, tmp[i]);
            for (size_type i = vec_count * simd_size; i < count; ++i)
                result = m_f(result, data[i]);
        }
        else
        {
            for (size_type i = 0; i < count; ++i) result = m_f(result, data[i]);
        }
        return result;
    }
    ...
    {
        return xreducer<F, E, X, ES>(std::forward<F>(f), std::forward<E>(e), std::forward<X>(axes), std::forward<ES>(es));
    }
