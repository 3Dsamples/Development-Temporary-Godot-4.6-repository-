...
// ViennaFixedJoint – welds two bodies together, eliminating all relative
// translational and rotational degrees of freedom.  Supports a breakable
// threshold and uses Baumgarte stabilisation for position errors.

#ifndef VIENNA_JOINTS_FIXED_H
#define VIENNA_JOINTS_FIXED_H
...
public:
	...
	// Set the relative transform from body A to body B that the joint must maintain.
...
protected:
	static void _bind_methods() {
		...
		ClassDB::bind_method(D_METHOD("set_breakable", "enabled"), &ViennaFixedJoint::set_breakable);
...
inline void ViennaFixedJoint::solve(ViennaBody *a, ViennaBody *b, real_t dt) {
	...
	if (invMassSum > CMP_EPSILON) {
		...
		totalForce = impulse / dt;  // approximate force
	}
