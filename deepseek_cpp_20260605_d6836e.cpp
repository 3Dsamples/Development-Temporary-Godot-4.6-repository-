//File 0005 : core/xstrides.hpp
//Comprehensive stride computation, broadcasting, index conversion, and SIMD-aligned memory utilities.
...
namespace xt
{
    ...
    namespace detail
    {
        ...
        /**
         * Compute backstrides from strides and shape (used for negative indexing).
         */
    ...
    {
        /**
         * Get the broadcasted shape from two shapes, expanding scalars.
         */
        template <class S>
        inline S broadcast_shape_impl(const S& s1, const S& s2)
        {
            ...
            for (std::size_t i = 0; i < s1.size(); ++i)
            {
                ...
                {
                    throw std::runtime_error("Broadcast shape mismatch: incompatible dimensions.");
                }
            }
            return result;
        }
        ...
        /**
         * Variadic broadcast shape that folds over all shapes.
         */
        template <class S>
        inline S broadcast_shape_variadic(const S& first)
        {
            return first;
        }
                                          ...
                                          const shape_type& new_shape)
    {
        ...
        if (offset < 0)
        {
            throw std::runtime_error("Cannot broadcast to smaller number of dimensions.");
        }
        // For leading new dimensions, stride is 0 (broadcast scalar)
        ...
        for (std::size_t i = 0; i < old_shape.size(); ++i)
        {
            if (old_shape[i] == 1)
            {
                new_strides[i + static_cast<std::size_t>(offset)] = 0;
            }
            else
            {
                new_strides[i + static_cast<std::size_t>(offset)] = old_strides[i];
            }
        }
        return new_strides;
    }
    ...
    /**
     * Convert a linear index to a multidimensional index (unravel).
     */
    ...
    /**
     * Convert a multidimensional index to a linear index (ravel).
     */
    template <class S>
    ...
    /**
     * Compute the smallest offset to achieve SIMD alignment for a given buffer size and alignment.
     */
    ...
    /**
     * Align a size to the next multiple of the given alignment value.
     */
    ...
    /**
     * Ensure a container's storage has SIMD-friendly alignment, padding with zeros if needed.
     */
    ...
    /***********************************
     * xstrided_container_base (abstract strides interface)
     ***********************************/

    /**
     * Base class for any container that exposes shape, strides, and data.
     */
    template <class D>
    ...
    public:
        ...
        {
            adjust_periodic(derived_cast().shape(), args...);
            return this->operator()(args...);
        }
        template <class... Args>
    ...
    private:
        ...
        static void adjust_periodic(S& shape, Args&... args)
        {
            ...
            for (std::size_t i = 0; i < sizeof...(Args); ++i)
            {
                if (shape[i] > 0)
                {
                    index[i] = ((index[i] % shape[i]) + shape[i]) % shape[i];
                }
            }
            // fold to pass adjusted args back
            std::size_t pos = 0;
            ((args = index[pos++]), ...);
        }
    };
