//File 0007 (UPDATED) : core/xstrided_view.hpp
//Dynamic rank strided view with slice support, dynamic layout deduction, SIMD element access, and full iterator/stepper integration.
...
namespace xt
{
    ...
    public:
        ...
        xstrided_view(E&& e, Slices&&... slices);
    ...
    // Constructor from explicit shape/strides
    template <class CT, class S, layout_type L, class FST>
    template <class E>
        ...
        , m_layout(layout)
    {
        ...
        // Deduce actual layout if dynamic
        if (m_layout == layout_type::dynamic)
            m_layout = deduce_layout(m_shape, m_strides);
    }

    // Constructor from slices (dynamic version)
    template <class CT, class S, layout_type L, class FST>
    template <class E, class... Slices>
    inline xstrided_view<CT, S, L, FST>::xstrided_view(E&& e, Slices&&... slices)
        : m_e(std::forward<E>(e))
        , m_offset(0)
        , m_layout(L)
    {
        ...
        // Compute sliced view shape/strides using xslice utilities
        auto [new_shape, new_strides] = compute_sliced_view(slice_tuple, old_shape, old_strides);
        m_shape = new_shape;
    ...
    public:
        ...
        reference operator*() const { return (*p_view).data()[m_offset]; }
    private:
        view_type* p_view;
        size_type m_offset;
    };
    ...
    public:
        ...
        const_reference operator*() const { return (*p_view).data()[m_offset]; }
    private:
        view_type* p_view;
        size_type m_offset;
    };
