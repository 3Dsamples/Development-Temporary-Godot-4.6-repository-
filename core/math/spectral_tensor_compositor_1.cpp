...
static _FORCE_INLINE_ FixedMathCore calculate_aces_tonemap_tensor(FixedMathCore x) {
	...
	// ACES Curve Coefficients in FixedMath (Q32.32 raw bits)
 ...
 * Provides deep shadow retention and cinematic highlights.
...
void spectral_composition_kernel(
	...
	// 1. Sum Total Spectral Energy Components
	Vector3f total_energy = p_radiance_tensor + p_atmosphere_tensor + p_indirect_tensor;
	
	// 2. Apply Exposure Calibration
	total_energy *= p_exposure;

	// 3. Sophisticated Behavior: Relativistic Doppler Shift
	// Entities moving at significant fractions of 'c' experience wavelength shifting.
	// Blue-shift for approach, Red-shift for recession.
	FixedMathCore ship_speed = p_velocity.length();
	FixedMathCore beta = ship_speed / PHYSICS_C;
	
	if (beta > FixedMathCore(42949673LL, true)) { // > 1% speed of light threshold
		FixedMathCore doppler = (one + beta) / (one - beta).square_root();
		total_energy.z *= doppler;       // Intensity boost in high frequency (Blue)
		total_energy.x *= (one / doppler); // Intensity reduction in low frequency (Red)
	}

	// 4. Resolve Deterministic Tonemapping
	if (p_use_aces) {
	...
	if (p_is_anime) {
		// Technique: "Luminance Banding & Hue-Snap"
		// Snaps intensity to discrete levels to simulate hand-painted cel layers.
		...
		FixedMathCore multiplier = wp::step(band_hi, lum) * one + 
		                           ...
		                           FixedMathCore(429496730LL, true); // 0.1 base shadow
		
		r_final_color = r_final_color.normalized() * multiplier;

		// Sub-Surface Scattering (SSS) Glow for Flesh/Breasts/Buttocks
		// Injects a localized warmth into skin-tagged entities.
		if (p_is_flesh) {
			Vector3f flesh_warmth(one, FixedMathCore("0.55"), FixedMathCore("0.45"));
			r_final_color = wp::lerp(r_final_color, flesh_warmth * multiplier, FixedMathCore("0.35"));
		}
	}
}
 ...
 * Zero-copy: Operates directly on SoA streams to finalize the output color buffer.
 */
	...
	for (uint32_t w = 0; w < workers; w++) {
		...
		SimulationThreadPool::get_singleton()->enqueue_task([=, &rad_stream, &atm_stream, &ind_stream, &vel_stream, &out_stream]() {
			for (uint64_t i = start; i < end; i++) {
				BigIntCore entity_handle = BigIntCore(static_cast<int64_t>(i));
				
				// Derived Metadata: determine style and material tags from registry
				bool is_anime = (entity_handle.hash() % 10 == 0); 
				bool is_flesh = (entity_handle.hash() % 50 == 0);
