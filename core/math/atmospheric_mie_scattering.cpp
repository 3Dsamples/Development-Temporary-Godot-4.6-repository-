--- START OF FILE core/math/atmospheric_mie_scattering.cpp ---
...
static _FORCE_INLINE_ FixedMathCore calculate_mie_phase_hg(FixedMathCore p_cos_theta, FixedMathCore p_g) {
	...
	FixedMathCore four_pi(13493037704LL, true); // 4 * PI in Q32.32 (3.1415... * 4 * 2^32)
	...
	if (unlikely(denominator.get_raw() == 0)) {
		return one; // Fallback for singularity at exact light vector
	}
 ...
 * 1. Ray-Marching: Samples density along the view vector using bit-perfect steps.
 * 2. Sun-Visibility: Performs a bit-perfect shadow check for each sample point.
 * 3. Haze Composition: Aggregates phase-shifted radiance based on local particle density.
 * 4. Anime Stylization: Snaps the Mie-glow into discrete intensity bands for cel-shading.
 */
void mie_volume_integrator_kernel(
	...
	for (int i = 0; i < sample_count; i++) {
		...
		// Altitude relative to planetary center
		FixedMathCore altitude = (sample_pos - p_params.planet_center).length() - p_params.planet_radius;

		if (altitude.get_raw() < 0) continue; // Sample is underground

		// 1. Resolve local haze density (Exponential decay: exp(-h / H))
		FixedMathCore density = (-(altitude / p_params.mie_scale_height)).exp();

		// 2. Deterministic Shadowing
		// Check if the sun is occluded by the planet at this specific atmospheric coordinate.
		// Uses wp:: intrinsics to ensure zero-copy logic.
		if (wp::check_occlusion_sphere(sample_pos, p_sun_dir, p_params.planet_center, p_params.planet_radius)) {
			continue;
		}

		// 3. Compute Phase Function (HG)
		FixedMathCore cos_theta = p_ray_dir.dot(p_sun_dir);
		FixedMathCore phase = calculate_mie_phase_hg(cos_theta, p_params.mie_g);

		// 4. --- Sophisticated Real-Time Behavior: Anime Style Resolve ---
		if (p_is_anime) {
			// Anime Technique: "Solar Rings". 
			// Quantize the Mie phase function into sharp bands to create stylized sun circles.
			FixedMathCore band_hi(3865470566LL, true); // 0.9 threshold
			FixedMathCore band_lo(2147483648LL, true); // 0.5 threshold
			
			if (phase > band_hi) {
				phase = FixedMathCore(8LL); // Intensified inner halo
			} else if (phase > band_lo) {
				phase = FixedMathCore(1717986918LL, true); // 0.4 secondary ring
			} else {
				phase = zero; // Sharp cel-shaded cutoff
			}
			
			// Saturation shift: move density toward a stylized constant
			density = (density > FixedMathCore(429496730LL, true)) ? one : zero;
		}

		// 5. Energy Integration
		// S_mie = Sun_I * Phase(theta) * Density(h) * Mie_Coeff
 ...
 * Partitions the EnTT radiance registry into worker chunks for the SimulationThreadPool.
 * Zero-copy: Operates directly on the aligned memory addresses of the registry SoA.
 */
void execute_mie_scattering_wave(
	...
	for (uint32_t w = 0; w < workers; w++) {
		...
		SimulationThreadPool::get_singleton()->enqueue_task([=, &radiance_stream, &pos_stream, &ray_stream, &len_stream]() {
			for (uint64_t i = start; i < end; i++) {
				// Deterministic Style selection based on entity handle hash (BigIntCore)
				BigIntCore entity_handle = p_registry.get_entity_at_index(COMPONENT_RADIANCE, i);
				bool anime_mode = (entity_handle.hash() % 12 == 0);
	...
	// Wait for synchronization barrier to finalize visual buffer
	SimulationThreadPool::get_singleton()->wait_for_all();
}

} // namespace UniversalSolver

--- END OF FILE core/math/atmospheric_mie_scattering.cpp ---
