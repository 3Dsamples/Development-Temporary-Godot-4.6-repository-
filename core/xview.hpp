...
// ----------------------------------------------------------------------------
// xview.hpp – Non‑owning multi‑dimensional view with slicing
// ----------------------------------------------------------------------------
// This header defines the xview class, which provides a non‑owning view of a
// subset of an xtensor expression. It supports:
//   - Integer indexing (reduces dimension)
//   - Range slicing (preserves dimension)
//   - New axis insertion (increases dimension)
//   - Ellipsis for multiple dimensions
//   - Full integration with BigNumber and FFT acceleration
// ----------------------------------------------------------------------------
...
namespace xt
{
    ...
    namespace detail
    {
        template <class T> struct is_xview : std::false_type {};
        template <class CT, class... S> struct is_xview<xview<CT, S...>> : std::true_type {};
        template <class T> inline constexpr bool is_xview_v = is_xview<T>::value;
    }

    // ========================================================================
    // Slicing types (range, all, newaxis, etc.)
    // ========================================================================
    ...
    public:
        ...
        xrange() noexcept;
        xrange(size_type stop) noexcept;
        xrange(size_type start, size_type stop) noexcept;
        xrange(size_type start, size_type stop, size_type step) noexcept;
        size_type start() const noexcept;
        size_type stop() const noexcept;
        size_type step() const noexcept;
        size_type size() const noexcept;
    private:
        size_type m_start, m_stop, m_step;
    };

    struct xall_tag {};
    static constexpr xall_tag all_tag = {};
    class xall { public: xall() noexcept = default; };

    struct xnewaxis_tag {};
    static constexpr xnewaxis_tag newaxis_tag = {};
    class xnewaxis { public: xnewaxis() noexcept = default; };

    struct xellipsis_tag {};
    static constexpr xellipsis_tag ellipsis_tag = {};
    class xellipsis { public: xellipsis() noexcept = default; };

    template <class T> class xkeep;
    template <class T> auto keep(T&& indices);
    template <class T> class xdrop;
    template <class T> auto drop(T&& indices);

    // ========================================================================
    // xview – Multi‑dimensional view with slicing
    // ========================================================================
    template <class CT, class... S>
    class xview : public xview_expression<xview<CT, S...>>
    {
    public:
        using self_type = xview<CT, S...>;
        using base_type = xview_expression<self_type>;
        using value_type = typename CT::value_type;
        using reference = std::conditional_t<CT::is_const, typename CT::const_reference, typename CT::reference>;
        using const_reference = typename CT::const_reference;
        using pointer = std::conditional_t<CT::is_const, typename CT::const_pointer, typename CT::pointer>;
        ...
        static constexpr bool is_const = CT::is_const;
        ...
        template <class E> self_type& assign(const xexpression<E>& e);
        template <class E> self_type& operator+=(const xexpression<E>& e);
        template <class E> self_type& operator-=(const xexpression<E>& e);
        template <class E> self_type& operator*=(const xexpression<E>& e);
        template <class E> self_type& operator/=(const xexpression<E>& e);
    ...
    private:
        ...
        size_type m_size;
        size_type m_data_offset;
        std::array<size_type, sizeof...(S)> m_integral_slices;

        shape_type make_shape() const;
        strides_type make_strides() const;
        size_type compute_size() const;
        size_type compute_data_offset() const;
        std::array<size_type, sizeof...(S)> compute_integral_slices() const;
        size_type compute_flat_offset(size_type flat_index) const;
        template <class... Args> size_type compute_offset(Args... args) const;
        template <class S> size_type compute_element_offset(const S& indices) const;
        template <class E> void assign_expression(const E& expr);
        template <class E> void assign_recursive(const E& expr, std::vector<size_type>& indices, size_type dim);
        template <class E, class Op> self_type& assign_composite(const xexpression<E>& e, Op&& op);
        template <class E, class Op> void assign_composite_recursive(const E& expr, std::vector<size_type>& indices, size_type dim, Op&& op);
    };

    // ------------------------------------------------------------------------
    // Helper traits for xkeep and xdrop
    // ------------------------------------------------------------------------
    template <class T> struct is_xkeep : std::false_type {};
    template <class T> struct is_xkeep<xkeep<T>> : std::true_type {};
    template <class T> inline constexpr bool is_xkeep_v = is_xkeep<T>::value;

    template <class T> struct is_xdrop : std::false_type {};
    template <class T> struct is_xdrop<xdrop<T>> : std::true_type {};
    template <class T> inline constexpr bool is_xdrop_v = is_xdrop<T>::value;

    // ========================================================================
    // Factory functions for creating views
    // ========================================================================
    template <class E, class... S> inline auto view(E&& e, S&&... slices);
    inline auto range(size_type stop);
    inline auto range(size_type start, size_type stop);
    inline auto range(size_type start, size_type stop, size_type step);
    static constexpr auto all = xall();
    static constexpr auto newaxis = xnewaxis();
    static constexpr auto ellipsis = xellipsis();

    // ------------------------------------------------------------------------
    // Inner types specialization for xview
    // ------------------------------------------------------------------------
    template <class CT, class... S>
...
{
    // xrange
    inline xrange::xrange() noexcept : m_start(0), m_stop(0), m_step(1) {}
    inline xrange::xrange(size_type stop) noexcept : m_start(0), m_stop(stop), m_step(1) {}
    inline xrange::xrange(size_type start, size_type stop) noexcept : m_start(start), m_stop(stop), m_step(1) {}
    inline xrange::xrange(size_type start, size_type stop, size_type step) noexcept : m_start(start), m_stop(stop), m_step(step) {}
    inline auto xrange::start() const noexcept -> size_type { return m_start; }
    inline auto xrange::stop() const noexcept -> size_type { return m_stop; }
    inline auto xrange::step() const noexcept -> size_type { return m_step; }
    inline auto xrange::size() const noexcept -> size_type
    { /* TODO: implement */ return 0; }

    // xkeep
    template <class T> class xkeep { public: using container_type = T; template <class U> explicit xkeep(U&& indices); const container_type& indices() const noexcept; size_type size() const noexcept; private: container_type m_indices; };
    template <class T> template <class U> xkeep<T>::xkeep(U&& indices) : m_indices(std::forward<U>(indices)) {}
    template <class T> auto xkeep<T>::indices() const noexcept -> const container_type& { return m_indices; }
    template <class T> auto xkeep<T>::size() const noexcept -> size_type { return m_indices.size(); }
    template <class T> auto keep(T&& indices) { return xkeep<std::decay_t<T>>(std::forward<T>(indices)); }

    // xdrop
    template <class T> class xdrop { public: using container_type = T; template <class U> explicit xdrop(U&& indices); const container_type& indices() const noexcept; private: container_type m_indices; };
    template <class T> template <class U> xdrop<T>::xdrop(U&& indices) : m_indices(std::forward<U>(indices)) {}
    template <class T> auto xdrop<T>::indices() const noexcept -> const container_type& { return m_indices; }
    template <class T> auto drop(T&& indices) { return xdrop<std::decay_t<T>>(std::forward<T>(indices)); }

    // xview
    template <class CT, class... S> template <class CTA, class... Args>
    xview<CT, S...>::xview(CTA&& e, Args&&... args)
        : m_expression(std::forward<CTA>(e)), m_slices(std::forward<Args>(args)...),
          m_shape(make_shape()), m_strides(make_strides()), m_size(compute_size()),
          m_data_offset(compute_data_offset()), m_integral_slices(compute_integral_slices())
    { /* TODO: implement */ }

    template <class CT, class... S> inline auto xview<CT, S...>::shape() const noexcept -> const shape_type& { return m_shape; }
    template <class CT, class... S> inline auto xview<CT, S...>::strides() const noexcept -> const strides_type& { return m_strides; }
    template <class CT, class... S> inline auto xview<CT, S...>::size() const noexcept -> size_type { return m_size; }
    template <class CT, class... S> inline auto xview<CT, S...>::dimension() const noexcept -> size_type { return m_shape.size(); }
    template <class CT, class... S> inline bool xview<CT, S...>::empty() const noexcept { return m_size == 0; }
    template <class CT, class... S> inline auto xview<CT, S...>::expression() noexcept -> CT& { return m_expression; }
    template <class CT, class... S> inline auto xview<CT, S...>::expression() const noexcept -> const CT& { return m_expression; }
    template <class CT, class... S> inline auto xview<CT, S...>::slices() const noexcept -> const slice_type& { return m_slices; }

    template <class CT, class... S> template <class... Args> inline auto xview<CT, S...>::operator()(Args... args) -> reference
    { size_type offset = compute_offset(args...); return m_expression.flat(offset); }
    template <class CT, class... S> template <class... Args> inline auto xview<CT, S...>::operator()(Args... args) const -> const_reference
    { size_type offset = compute_offset(args...); return m_expression.flat(offset); }
    template <class CT, class... S> inline auto xview<CT, S...>::operator[](size_type i) -> reference { return flat(i); }
    template <class CT, class... S> inline auto xview<CT, S...>::operator[](size_type i) const -> const_reference { return flat(i); }
    template <class CT, class... S> inline auto xview<CT, S...>::flat(size_type i) -> reference
    { size_type offset = compute_flat_offset(i); return m_expression.flat(offset); }
    template <class CT, class... S> inline auto xview<CT, S...>::flat(size_type i) const -> const_reference
    { size_type offset = compute_flat_offset(i); return m_expression.flat(offset); }
    template <class CT, class... S> template <class S> inline auto xview<CT, S...>::element(const S& indices) -> reference
    { size_type offset = compute_element_offset(indices); return m_expression.flat(offset); }
    template <class CT, class... S> template <class S> inline auto xview<CT, S...>::element(const S& indices) const -> const_reference
    { size_type offset = compute_element_offset(indices); return m_expression.flat(offset); }

    template <class CT, class... S> inline auto xview<CT, S...>::data() noexcept -> pointer { return m_expression.data() + m_data_offset; }
    template <class CT, class... S> inline auto xview<CT, S...>::data() const noexcept -> const_pointer { return m_expression.data() + m_data_offset; }

    template <class CT, class... S> template <class E> inline auto xview<CT, S...>::operator=(const xexpression<E>& e) -> self_type& { return assign(e); }
    template <class CT, class... S> template <class E> inline auto xview<CT, S...>::assign(const xexpression<E>& e) -> self_type& { /* TODO: implement */ return *this; }
    template <class CT, class... S> template <class E> inline auto xview<CT, S...>::operator+=(const xexpression<E>& e) -> self_type& { return assign_composite(e, detail::plus()); }
    template <class CT, class... S> template <class E> inline auto xview<CT, S...>::operator-=(const xexpression<E>& e) -> self_type& { return assign_composite(e, detail::minus()); }
    template <class CT, class... S> template <class E> inline auto xview<CT, S...>::operator*=(const xexpression<E>& e) -> self_type&
    ...
    template <class CT, class... S> template <class E> inline auto xview<CT, S...>::operator/=(const xexpression<E>& e) -> self_type& { return assign_composite(e, detail::divides()); }

    // iterator classes (stubs)
    template <class CT, class... S> class xview<CT, S...>::iterator { public: /* TODO: implement */ };
    template <class CT, class... S> class xview<CT, S...>::const_iterator { public: /* TODO: implement */ };
    template <class CT, class... S> inline auto xview<CT, S...>::begin() -> iterator { return iterator(this, 0); }
    template <class CT, class... S> inline auto xview<CT, S...>::end() -> iterator { return iterator(this, m_size); }
    template <class CT, class... S> inline auto xview<CT, S...>::begin() const -> const_iterator { return const_iterator(this, 0); }
    template <class CT, class... S> inline auto xview<CT, S...>::end() const -> const_iterator { return const_iterator(this, m_size); }
    template <class CT, class... S> inline auto xview<CT, S...>::cbegin() const -> const_iterator { return begin(); }
    template <class CT, class... S> inline auto xview<CT, S...>::cend() const -> const_iterator { return end(); }

    // Private helpers (stubs)
    template <class CT, class... S> auto xview<CT, S...>::make_shape() const -> shape_type { /* TODO: implement */ return {}; }
    template <class CT, class... S> auto xview<CT, S...>::make_strides() const -> strides_type { /* TODO: implement */ return {}; }
    template <class CT, class... S> auto xview<CT, S...>::compute_size() const -> size_type { /* TODO: implement */ return 0; }
    template <class CT, class... S> auto xview<CT, S...>::compute_data_offset() const -> size_type { /* TODO: implement */ return 0; }
    template <class CT, class... S> auto xview<CT, S...>::compute_integral_slices() const -> std::array<size_type, sizeof...(S)> { /* TODO: implement */ return {}; }
    template <class CT, class... S> auto xview<CT, S...>::compute_flat_offset(size_type flat_index) const -> size_type { /* TODO: implement */ return flat_index; }
    template <class CT, class... S> template <class... Args> auto xview<CT, S...>::compute_offset(Args... args) const -> size_type { /* TODO: implement */ return 0; }
    template <class CT, class... S> template <class S> auto xview<CT, S...>::compute_element_offset(const S& indices) const -> size_type { /* TODO: implement */ return 0; }
    template <class CT, class... S> template <class E> void xview<CT, S...>::assign_expression(const E& expr) { /* TODO: implement */ }
    template <class CT, class... S> template <class E> void xview<CT, S...>::assign_recursive(const E& expr, std::vector<size_type>& indices, size_type dim) { /* TODO: implement */ }
    template <class CT, class... S> template <class E, class Op> auto xview<CT, S...>::assign_composite(const xexpression<E>& e, Op&& op) -> self_type& { /* TODO: implement */ return *this; }
    template <class CT, class... S> template <class E, class Op> void xview<CT, S...>::assign_composite_recursive(const E& expr, std::vector<size_type>& indices, size_type dim, Op&& op) { /* TODO: implement */ }

    // Factory functions
    template <class E, class... S> inline auto view(E&& e, S&&... slices)
    { return xview<std::decay_t<E>, std::decay_t<S>...>(std::forward<E>(e), std::forward<S>(slices)...); }
    inline auto range(size_type stop) { return xrange(stop); }
    inline auto range(size_type start, size_type stop) { return xrange(start, stop); }
    inline auto range(size_type start, size_type stop, size_type step) { return xrange(start, stop, step); }

} // namespace xt

#endif // XTENSOR_XVIEW_HPP    private:
        CT m_expression;
        slice_type m_slices;
        shape_type m_shape;
        strides_type m_strides;
        ...
        shape_type make_shape() const
        {
            ...
            size_type expr_dim = m_expression.dimension();
            size_type slice_idx = 0;
            size_type expr_idx = 0;

            auto process_slice = [&](const auto& slice)
            {
                using slice_t = std::decay_t<decltype(slice)>;
                if constexpr (std::is_same_v<slice_t, size_type>)
                {
                    // Integer indexing: dimension is dropped
                    ++expr_idx;
                }
                else if constexpr (std::is_same_v<slice_t, xrange>)
                {
                    new_shape.push_back(slice.size());
                    ++expr_idx;
                }
                else if constexpr (std::is_same_v<slice_t, xall>)
                {
                    new_shape.push_back(m_expression.shape()[expr_idx]);
                    ++expr_idx;
                }
                else if constexpr (std::is_same_v<slice_t, xnewaxis>)
                {
                    new_shape.push_back(1);
                    // expr_idx unchanged
                }
                else if constexpr (std::is_same_v<slice_t, xellipsis>)
                {
                    // Handle ellipsis: fill remaining dimensions
                    size_t remaining_slices = sizeof...(S) - slice_idx - 1;
                    size_t remaining_expr_dims = expr_dim - expr_idx;
                    size_t fill_dims = remaining_expr_dims - remaining_slices;
                    for (size_t i = 0; i < fill_dims; ++i)
                    {
                        new_shape.push_back(m_expression.shape()[expr_idx + i]);
                    }
                    expr_idx += fill_dims;
                }
                else if constexpr (is_xkeep_v<slice_t>)
                {
                    new_shape.push_back(slice.size());
                    ++expr_idx;
                }
                else if constexpr (is_xdrop_v<slice_t>)
                {
                    size_type drop_size = slice.indices().size();
                    size_type original_size = m_expression.shape()[expr_idx];
                    new_shape.push_back(original_size - drop_size);
                    ++expr_idx;
                }
            };

            (process_slice(std::get<slice_idx>(m_slices)), ++slice_idx, ...);
        ...
        strides_type make_strides() const
        {
            strides_type new_strides(m_shape.size(), 0);
            const auto& expr_strides = m_expression.strides();
            size_type expr_dim = m_expression.dimension();
            size_type view_dim = 0;
            size_type slice_idx = 0;
            size_type expr_idx = 0;
            ...
            auto process_slice = [&](const auto& slice)
            {
                ...
                {
                    new_strides[view_dim] = expr_strides[expr_idx] * slice.step();
                    ++view_dim;
                    ++expr_idx;
                }
                else if constexpr (std::is_same_v<slice_t, xall>)
                {
                    new_strides[view_dim] = expr_strides[expr_idx];
                    ++view_dim;
                    ++expr_idx;
                }
                else if constexpr (std::is_same_v<slice_t, xnewaxis>)
                {
                    new_strides[view_dim] = 0;
                    ++view_dim;
                }
                else if constexpr (std::is_same_v<slice_t, xellipsis>)
                {
                    ...
                    for (size_t i = 0; i < fill_dims; ++i)
                    {
                        new_strides[view_dim] = expr_strides[expr_idx + i];
                        ++view_dim;
                    }
                    expr_idx += fill_dims;
                }
        ...
        size_type compute_data_offset() const
        {
            ...
            auto process_slice = [&](const auto& slice)
            {
                ...
                if constexpr (std::is_same_v<slice_t, size_type>)
                {
                    offset += slice * m_expression.strides()[expr_idx];
                    ++expr_idx;
                }
                else if constexpr (std::is_same_v<slice_t, xrange>)
                {
                    offset += slice.start() * m_expression.strides()[expr_idx];
                    ++expr_idx;
                }
                else if constexpr (std::is_same_v<slice_t, xall>)
                {
                ...
                {
                    // Skip dimensions that are filled by ellipsis
                    size_t remaining_slices = sizeof...(S) - slice_idx - 1;
                    size_t remaining_expr_dims = m_expression.dimension() - expr_idx;
                    size_t fill_dims = remaining_expr_dims - remaining_slices;
                    expr_idx += fill_dims;
                }
                else if constexpr (is_xkeep_v<slice_t>)
                {
                    // Use first kept index as base offset (others handled during indexing)
                    if (slice.size() > 0)
                    {
                        offset += slice.indices()[0] * m_expression.strides()[expr_idx];
                    }
                    ++expr_idx;
                }
                else if constexpr (is_xdrop_v<slice_t>)
        ...
        // --------------------------------------------------------------------
        // Integral slice indices (for mapping)
        // --------------------------------------------------------------------
        std::array<size_type, sizeof...(S)> compute_integral_slices() const
        {
            std::array<size_type, sizeof...(S)> result{};
            size_type slice_idx = 0;
        ...
        // --------------------------------------------------------------------
        // Offset computation for flat index
        // --------------------------------------------------------------------
        size_type compute_flat_offset(size_type flat_index) const
        {
            size_type offset = m_data_offset;
            size_type remaining = flat_index;

            for (size_type d = m_shape.size(); d-- > 0; )
            {
                size_type stride = m_strides[d];
                size_type coord = remaining % m_shape[d];
                remaining /= m_shape[d];
                offset += coord * stride;
            }
        ...
        // --------------------------------------------------------------------
        // Offset from multi‑dimensional indices
        // --------------------------------------------------------------------
        template <class... Args>
        size_type compute_offset(Args... args) const
        {
            std::array<size_type, sizeof...(Args)> indices = {static_cast<size_type>(args)...};
            size_type offset = m_data_offset;
            for (size_type d = 0; d < m_shape.size(); ++d)
            {
                offset += indices[d] * m_strides[d];
            }
            return offset;
        }

        template <class S>
        size_type compute_element_offset(const S& indices) const
        {
            size_type offset = m_data_offset;
            for (size_type d = 0; d < m_shape.size(); ++d)
            {
        ...
        // --------------------------------------------------------------------
        template <class E>
        void assign_expression(const E& expr)
        {
            if (m_shape == expr.shape())
            {
            ...
            else
            {
                ...
                std::vector<size_type> indices(m_shape.size(), 0);
                assign_recursive(expr, indices, 0);
            }
        }

        template <class E>
        void assign_recursive(const E& expr, std::vector<size_type>& indices, size_type dim)
        {
            if (dim == m_shape.size())
            {
                element(indices) = expr.element(indices);
                return;
            }

            for (size_type i = 0; i < m_shape[dim]; ++i)
            {
                indices[dim] = i;
                assign_recursive(expr, indices, dim + 1);
            }
        }
        ...
        self_type& assign_composite(const xexpression<E>& e, Op&& op)
        {
            ...
            if (m_shape == expr.shape())
            {
                for (size_type i = 0; i < m_size; ++i)
                {
                    flat(i) = op(flat(i), expr.flat(i));
                }
            }
            else
            {
                std::vector<size_type> indices(m_shape.size(), 0);
                assign_composite_recursive(expr, indices, 0, op);
            }
            return *this;
        }
                                        ...
                                        size_type dim, Op&& op)
        {
            if (dim == m_shape.size())
            {
                reference ref = element(indices);
                ref = op(ref, expr.element(indices));
                return;
            }
            ...
            {
                indices[dim] = i;
                assign_composite_recursive(expr, indices, dim + 1, op);
            }
        }
    };
    ...
    // ------------------------------------------------------------------------
    template <class T>
    struct is_xkeep : std::false_type {};

    template <class T>
    struct is_xkeep<xkeep<T>> : std::true_type {};

    template <class T>
    inline constexpr bool is_xkeep_v = is_xkeep<T>::value;

    template <class T>
    struct is_xdrop : std::false_type {};

    template <class T>
    struct is_xdrop<xdrop<T>> : std::true_type {};

    template <class T>
    inline constexpr bool is_xdrop_v = is_xdrop<T>::value;
    ...
    inline auto view(E&& e, S&&... slices)
    {
        ...
        return xview<expr_type, std::decay_t<S>...>(
            std::forward<E>(e), std::forward<S>(slices)...
        );
    }

    // Convenience aliases for common slicing operations
    inline auto range(size_type stop) { return xrange(stop); }
    inline auto range(size_type start, size_type stop) { return xrange(start, stop); }
    inline auto range(size_type start, size_type stop, size_type step) { return xrange(start, stop, step); }
