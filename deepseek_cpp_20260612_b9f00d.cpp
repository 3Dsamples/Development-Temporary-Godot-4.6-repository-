...
// Description : Implementation of DecalExt with texture maps, size, distance/angle fade,
//               normal mapping, culling, GI contribution, and full RenderingServer sync.
#include "decal_ext.h"
...
struct DecalExt::Impl {
    ...
    void sync() {
        ...
        rs->decal_set_texture(decal_rid, RS::DECAL_TEXTURE_ALBEDO, albedo_texture);
        rs->decal_set_texture(decal_rid, RS::DECAL_TEXTURE_NORMAL, normal_texture);
        rs->decal_set_texture(decal_rid, RS::DECAL_TEXTURE_ORM, orm_texture);
        rs->decal_set_texture(decal_rid, RS::DECAL_TEXTURE_EMISSIVE, emissive_texture);
        rs->decal_set_albedo(decal_rid, albedo_color);
        ...
        rs->decal_set_cull_angle(decal_rid, Math::deg_to_rad(cull_angle));
        rs->decal_set_gi_mode(decal_rid, gi_mode);
        rs->decal_set_gi_contribution(decal_rid, gi_contribution);
        rs->decal_set_distance_fade(decal_rid, distance_fade_enabled, distance_fade_begin, distance_fade_end);
        rs->decal_set_angle_fade(decal_rid, angle_fade_enabled, Math::deg_to_rad(angle_fade_threshold));
        dirty = false;
    }
};
...
void DecalExt::set_albedo(const Color &p_color) {
    pimpl->albedo_color = p_color;
    pimpl->dirty = true;
    sync_decal();
}
Color DecalExt::get_albedo() const { return pimpl->albedo_color; }
...
Color DecalExt::get_emissive_color() const { return pimpl->emissive_color; }
...
void DecalExt::set_cull_angle(float p_angle_deg) {
    pimpl->cull_angle = p_angle_deg;
    pimpl->dirty = true;
    sync_decal();
}
float DecalExt::get_cull_angle() const { return pimpl->cull_angle; }
