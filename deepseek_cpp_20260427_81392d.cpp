// File 330: modules/wicked/src/bodies/wicked_body.h
// WickedBody – rigid body for the WickedEngine physics module.
// Stores mass, inertia, velocity, damping, forces, collision shape, material,
// activation state, and CCD flag. Integrates with Godot's ref-counted system.

#ifndef WICKED_BODIES_WICKED_BODY_H
#define WICKED_BODIES_WICKED_BODY_H
...
class WickedBody : public RefCounted {
    GDCLASS(WickedBody, RefCounted);
...
public:
    ...
    // --- Collision shape ---
    void set_collision_shape(const Ref<WickedShape> &p_shape);
    Ref<WickedShape> get_collision_shape() const { return collision_shape; }
    ...
    void apply_torque_impulse(const vec3 &p_torque);
    void clear_forces();
    ...
    // --- Activation / sleep ---
    void set_activation_state(ActivationState p_state);
    ActivationState get_activation_state() const { return activation_state; }
    void activate(bool p_force = false);
    void deactivate();
    void set_sleep_counter(int p_count) { sleep_counter = p_count; }
    ...
    // --- CCD ---
    void set_ccd_enabled(bool p_enabled) { ccd_enabled = p_enabled; }
    bool is_ccd_enabled() const { return ccd_enabled; }
    void set_ccd_motion_threshold(real_t p_threshold);
    real_t get_ccd_motion_threshold() const { return ccd_motion_threshold; }
    void set_ccd_swept_sphere_radius(real_t p_radius);
    real_t get_ccd_swept_sphere_radius() const { return ccd_swept_sphere_radius; }

    // --- Deactivation control ---
    void set_deactivation_enabled(bool p_enabled) { deactivation_enabled = p_enabled; }
    bool is_deactivation_enabled() const { return deactivation_enabled; }
...
private:
    ...
    mat3 inertia_local;                // local frame inertia tensor
    mat3 inverse_inertia_local;        // inv(inertia_local)
    mat3 inverse_inertia_world;        // R * inv_inertia_local * R^T
    aabb cached_aabb;
    Ref<WickedShape> collision_shape;
    material_id mat_id = 0;
    ActivationState activation_state = ActivationState::ACTIVE_TAG;
    int sleep_counter = 0;
    bool gravity_enabled = true;
    bool deactivation_enabled = true;
    bool ccd_enabled = false;
    real_t ccd_motion_threshold = 0.0;
    real_t ccd_swept_sphere_radius = 0.2;
...
#endif // WICKED_BODIES_WICKED_BODY_H