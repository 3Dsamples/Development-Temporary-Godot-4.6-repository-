// File 220: modules/newton/src/world/newton_world.h (update)
// Updated NewtonWorld declaration with iteration accessors for bodies,
// joints, and materials. Includes all previously declared members and
// adds public methods required for serialisation and contact generation.
...
public:
	...
	// --- Collision pair storage (used between broad‑phase and solver) ---
	void add_contact_pair(body_id a, body_id b);
	void clear_contact_pairs();
	const LocalVector<std::pair<body_id, body_id>> &get_contact_pairs() const;

	// --- Contact generation (narrow‑phase) ---
	void generate_contacts_for_pair(body_id a, body_id b);
	LocalVector<NewtonContactPoint> &get_generated_contacts() { return generated_contacts; }
...
private:
	...
	// Overlapping pairs from broad-phase
	LocalVector<std::pair<body_id, body_id>> contact_pairs;

	// Generated contacts from narrow-phase
	LocalVector<NewtonContactPoint> generated_contacts;
